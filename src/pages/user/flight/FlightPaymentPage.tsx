...
import { Flight, FlightSearch, PassengerDetail, FlightBooking } from '../../../types';
...
export default function FlightPaymentPage() {
  ...
  const { flight, search, travellers } = location.state as {
    flight: Flight; search: FlightSearch; travellers: PassengerDetail[];
  };

  const travellersCount = search.travellers || 1;
  const baseAmount = flight.price * travellersCount;
  
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>('upi');
  const [selectedOffer, setSelectedOffer] = useState(offers[1]); // Default to a flight offer if possible
  ...
  const handlePay = async () => {
    ...
    const booking: FlightBooking = {
      type: 'flight', bookingId: generateBookingId(), pnr: generatePNR(),
      flight, cabinClass: search.cabinClass, travellers, totalAmount: baseAmount, discount,
      finalAmount, paymentMethod, bookingDate: new Date().toISOString(),
      travelDate: search.date, status: 'confirmed',
    };

    addBooking(booking);
    toast.success('Booking confirmed!');
    await new Promise((r) => setTimeout(r, 1200));
    navigate('/flight/success', { state: { booking }, replace: true });
  };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8F9FB' }}>
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200 }} style={{ textAlign: 'center' }}>
          <div style={{ margin: '0 auto 16px', width: 96, height: 96, background: '#DBEAFE', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FB', paddingBottom: 100 }}>
      ...
      <UserHeader title="Payment" subtitle="Complete flight booking" showBack />
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 24px', display: 'grid', gridTemplateColumns: '1fr', gap: 20 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'Inter', fontSize: 14, color: '#4B5563' }}>{travellersCount} Traveller(s) × ₹{flight.price.toLocaleString()}</span>
              <span style={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: 14, color: '#111827' }}>₹{baseAmount.toLocaleString()}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'Inter', fontSize: 14, color: '#4B5563' }}>Taxes & Fees</span>
              <span style={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: 14, color: '#10B981' }}>Included</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'Inter', fontSize: 14, color: '#4B5563' }}>Discount ({selectedOffer.code})</span>
              <span style={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: 14, color: '#10B981' }}>-₹{discount.toLocaleString()}</span>
            </div>
            <div style={{ height: 1, background: '#E5E7EB', margin: '8px 0' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'Poppins', fontWeight: 700, fontSize: 16, color: '#111827' }}>Total Payable</span>
              <span style={{ fontFamily: 'Poppins', fontWeight: 800, fontSize: 20, color: '#DC2626' }}>₹{finalAmount.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {offers.filter((o) => o.applicableTo === 'flight' || o.applicableTo === 'both').map((offer) => (
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #F3F4F6', padding: '16px 24px', boxShadow: '0 -4px 20px rgba(0,0,0,0.08)', zIndex: 50 }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <motion.button id="pay-now-btn" whileTap={!processing ? { scale: 0.98 } : {}} onClick={handlePay} disabled={processing}
            ...
            {processing ? <Loader2 size={20} className="animate-spin" /> : `Pay ₹${finalAmount.toLocaleString()} Securely`}
          </motion.button>
        </div>
      </div>
    </div>
